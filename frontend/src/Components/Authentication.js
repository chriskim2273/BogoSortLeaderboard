...
function AuthenticationComponent(props) {
    ...
    const scoresComponent = loadingScores && !userScores ? <Skeleton m={5} height='100%' /> : <OrderedList>
        {scoresArray ? scoresArray.map((score_element, index) => {
            ...
            return (<Box key={"leaderboard_box_" + String(amount_of_elements) + "_" + String(index)} paddingTop={4}><ListItem key={"leaderboard_" + String(amount_of_elements) + "_" + String(index)}>N: {amount_of_elements} - θ({score}) - {time_of_score}</ListItem><Divider paddingTop={2} paddingBottom={2} /></Box>)
